...
void systemInit(){ //initialize the sybsystems 
    engineInit();
    displayInit();
    displaySetup();
    wiperInit();
...
void systemUpdate(){ //Loop that continually updates the subsystems of the design
    engineUpdate();
    displayUpdate();
    if(readEngine()){
        updateWipers();
...
static void displaySetup(){ //initilaize the LCD display
...
static void displayUpdate(){ //update what the module displays as the mode and delay changes 
    switch(readMode()){
        case 0 : 
