...
#define MODE_HI 0.75 //Settings of mode potentiometer
#define MODE_INT 0.5
#define MODE_LO 0.25

#define DELAY_SHORT 0.33 //Settings of delay potentiometer
#define DELAY_LONG 0.66

#define PERIOD 0.02 //Motor settings of low and high position
#define DUTY_MAX 0.110
#define DUTY_MIN 0.08

#define INCREMENT 50 //amount of increments the low state cycles through
#define DELAY_TIME_MS 500 //amount of time it takes for the low mode to sweep one way
...
void updateMode(){ //update the current mode
...
void updateDelay(){ //update the current delay
...
static void cycleWipers(){ //FSM that implements cycling the wipers 
   ...
   switch(wiperState){
        ...
        case WAIT : //implements the wait time in the inctermittent mode
            switch(wiperDelay){
                case 0 :  
                delay(2000); break;
                case 1 : 
...
static void sweep(){ //implements the slow sweep wiper setting
    float temp;
